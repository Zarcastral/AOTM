            ...
            if (user) {
                try {
                    ...
                    if (!userSnapshot.empty) {
                        ...
                        console.error("User record not found in tb_users for email:", user.email);
...
async function deleteFarmerAccount(farmer_id) {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            showDeleteMessage("No Farmer ID found, Unable to proceed with deleting the record", false);
        }
    } catch (error) {
    ...
    if (selectedRowId) {
        try {
            ...
            if (farmerDocSnap.exists()) {
                ...
                // Log the activity
                await saveActivityLog("Delete", `Archived and deleted farmer account ${farmerData.farmer_id} by ${userType} ${userName}`);

                // Refresh the table and show success message
                fetchFarmerAccounts();
                showDeleteMessage("Farmer account has been archived and deleted!", true);
            }
        } catch (error) {
