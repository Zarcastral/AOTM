...
async function getAuthenticatedUser() {
  return new Promise((resolve, reject) => {
    ...
    async function fetchHarvest() {
        ...
        // Get user_type from the fetched user document (not used now, but kept for consistency)
        ...
        onSnapshot(harvestQuery, async (snapshot) => {
            ...
            // Process records for expiration
            await processExpiredRecords(harvestData);
    
            // Update the UI with remaining records
    ...
    // Helper function to check if a record is older than 6 months
    function isExpired(harvestDate) {
        if (!harvestDate) return false; // No date, assume not expired
    
        const date = harvestDate.toDate ? harvestDate.toDate() : new Date(harvestDate); // Handle Firestore Timestamp or string
        const now = new Date(); // Current date (March 23, 2025, per your setup)
        const sixMonthsAgo = new Date(now.setMonth(now.getMonth() - 6)); // 6 months ago from now
    
        return date < sixMonthsAgo; // True if harvest_date is older than 6 months
    }
    
    // Process records that have expired (copy to history and delete)
    async function processExpiredRecords(harvestData) {
        ...
        if (isExpired(harvest.harvest_date)) {
            // Step 1: Duplicate to tb_harvest_history
            const historyDocRef = doc(historyCollection, harvest.id); // Use same ID for simplicity
            batch.set(historyDocRef, harvest);
    
            // Step 2: Mark for deletion from tb_harvest
