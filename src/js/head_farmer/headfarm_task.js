...
/**
 * Fetch project tasks and allow status updates.
 */
...
/**
 * Update task status in Firestore when changed from dropdown.
 * - If status is changed to "Ongoing", set `start_date` if it doesn’t exist.
 * - If status is changed to "Completed", set `end_date`.
 * - If status is changed back to "Ongoing" or "Pending", remove `end_date` but keep `start_date`.
 * - If status is changed to "Pending", remove `start_date` **only if `end_date` is also removed**.
 */
async function updateTaskStatus(taskId, newStatus) {
  try {
    ...
    if (newStatus === "Ongoing") {
      // Keep start_date if it already exists, otherwise set it
      updateData.start_date = today;
      updateData.end_date = null; // Remove end_date when switching from Completed
    } else if (newStatus === "Pending") {
      // Remove end_date, but only remove start_date if it has no end_date
      updateData.end_date = null;
      updateData.start_date = null;
    } else if (newStatus === "Completed") {
      // Set end_date, but keep start_date
...
saveTaskBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    await addDoc(tasksRef, {
      task_name: taskName,
      project_id: String(projectId), // Ensure project_id is stored as a string
